...
public class WinnerChecker {
     ...
     *
     * @param position -enum to specify search position
     * @param engine
    ...
    public static Player checkTheWinner(BoardWinnerPositions position, GameManager engine) {
        ...
        } else if (position == BoardWinnerPositions.EDGE) {
            return CheckWinnerFromEdges(board);
        }
    ...
    private static Player CheckWinnerFromEdges(Board board) {
        if (
                (board.getPlayerAtGivenPosition(new int[]{0, 0})
                        == board.getPlayerAtGivenPosition(new int[]{0, GameManager.getBoardSize() - 1}))
                        &&
                        (board.getPlayerAtGivenPosition(new int[]{0, 0})
                                == board.getPlayerAtGivenPosition(new int[]{GameManager.getBoardSize() - 1, 0}))
                        && (board.getPlayerAtGivenPosition(new int[]{GameManager.getBoardSize() - 1, 0})
                        == board.getPlayerAtGivenPosition(new int[]{GameManager.getBoardSize() -1, GameManager.getBoardSize() - 1}))) {
            return board.getPlayerAtGivenPosition(new int[]{0, 0});
        }
        return null;
    }
